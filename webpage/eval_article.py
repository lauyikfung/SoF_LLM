...
def get_analysis(s: str):
  try:
    ...
    try:
      score = re.findall(r'\d{2}', s)[-1]
      score = int(score)
      return {"analysis": s, "result": score}
    except:
      return {"analysis": "", "result": -1}
       
def eval_article(query, answer):
    eval_prompt = eval_template.replace('[[TIMU0]]',query).replace('[[FANWEN0]]', answer).replace("[[WC]]", str(len(answer))).replace("[[WC_RESULT]]", get_wc_compare_results(answer))
    eval_result = get_response(eval_prompt)
    analysis_result = get_analysis(eval_result)
    return analysis_result['result'], "<br><br>".join(analysis_result['analysis']).replace("范文0", "本文").replace("范文", "文章")