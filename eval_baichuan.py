...
class Seq2SeqTrainingArguments(Seq2SeqTrainingArguments):
    ...
    project_name: str = field(default="baichuan")
    run_name: str = field(default="baichuan")
...
class DataArguments:
    ...
    template_name: Optional[str] = field(default="llama", metadata={"help": "The prompt template name."})
...
def main():
    ...
    if training_args.report_to == "wandb":
        wandb.init(project=training_args.project_name+time.strftime("%Y%m%d-%H%M%S",time.gmtime()), 
                   name=training_args.run_name+time.strftime("%Y%m%d-%H%M%S",time.gmtime()),
                   config=config,
                   resume='None')
    ...
    if model_args.model_name_or_path:
        ...
        if model_args.flash_attn:
            if getattr(config, "model_type", None) in ["llama", "baichuan"]:
        ...
        elif model_args.shift_attn and getattr(config, "model_type", None) in ["llama", "baichuan"]:
            modeling_llama.LlamaAttention = LlamaShiftShortAttention
        if model_args.shift_attn:
            if getattr(config, "model_type", None) in ["llama"]:
    ...
    if training_args.do_eval:
        ...
        with torch.no_grad():
          for item in tqdm(eval_dataset):
            ...
            input_ids = input_ids[labels == -100].unsqueeze(0)
            query = tokenizer.decode(input_ids.tolist()[0])
            input_ids = input_ids.to(training_args.device)
            generated = model.generate(input_ids=input_ids, max_length=2048, use_cache=False, num_return_sequences=1, early_stopping=True, num_beams=1, no_repeat_ngram_size=13, repetition_penalty=1.5)
            input_ids = input_ids.to("cpu")##
            torch.cuda.empty_cache()
            del input_ids
            result = tokenizer.decode(generated[0].tolist(), skip_special_tokens=True)
            result_pair.append({"query": query, "response": result})
        with open("prediction.jsonl", "w") as f:
            jsonlines.Writer(f).write_all(result_pair)
